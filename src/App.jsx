...
import { VisibilityIcon, WindIcon,HumidityIcon, SunriseIcon, SunsetIcon } from './components/icons';
...
const API_KEY = '94c8e0fd4d822a98ad740b2010642cb0'
...
//THIS FUNCTION PREVENTS FORM SUBMISSION VALIDATES CITY AND FETCHES
//  DATA VIA API 
...
{!weather ? (
  <form onSubmit={handleSearch} className='flex flex-col relative'>
    ...
    />
    {suggestion.length > 0 && (
      <div className='absolute top-12 left-0 right-0 bg-transparent
...
{suggestion.map((s) => (
  ...
  )} className='block hover:bg-blue-700 bg-transparent px-4 py-2 
...
{s.name}, {s.country}{s.state && `, ${s.state}`}
    ...
    <button type='submit' className='bg-purple-700 hover:bg-blue-700
...
<button onClick={() => {setWeather(null); setCity('')}}
  className='mb-4 bg-purple-900 hover:bg-blue-700 text-white 
  font-semibold py-1 px-3 rounded transition-colors'
  >
New Search
</button>
